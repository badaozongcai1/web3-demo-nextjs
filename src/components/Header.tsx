            ...
            <nav className="flex items-center space-x-6">
              {[
                ...
                >
                  <Link
                    href={item.href}
                    className="relative px-3 py-2 text-gray-600 transition-colors group"
                  >
                    <span className="relative z-10">{item.label}</span>
                    <div className="absolute inset-0 h-full w-full bg-gradient-to-r from-violet-100 to-purple-100 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity -z-1" />
                  </Link>
                </motion.div>
              ))}
            </nav>
